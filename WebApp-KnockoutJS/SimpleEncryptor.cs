...
public class SimpleEncryptor2
{
    ...
    public string Encrypt(string plainValue)
    {
        ...
        // 대상 값 길이가 keyset 길이보다 크면 keyset크기를 동일하게 맞춘다.            
        var keyCnt = plainValue.Length > _keyCount ? plainValue.Length : _keyCount;
        var keys = GetRandomKeys(keyCnt);

        bool addedBOM = false;
        int chipherIdx = 0, wordIdx = 0;
        
        var totalLength = plainValue.Length + keys.Length;

        var arr = new char[totalLength];
        for (int i = 0; i < totalLength; i++)
        {
            // 실제 값을 모두 채운 상태에서 빈 배열이 존재할 경우 임의의 바이트를 삽입한다.
            if (wordIdx >= plainValue.Length && !addedBOM)
            {
                arr[i] = flag;
                addedBOM = true;
                continue;
            ...
            // 짝수(0자리 포함) 자리에 할당된 keyset만큼 채운다.
                ...
                arr[i] = keys[chipherIdx++];
            // 홀수자리에 실제 값을 채운다.
            else if (wordIdx < plainValue.Length)
                arr[i] = plainValue[wordIdx++];
        }
        // 배열을 뒤집는다.
        Array.Reverse(arr);
        var ret = Convert.ToBase64String(Encoding.UTF8.GetBytes(arr));
        // 문자열로 치환.
        //var ret = string.Join(string.Empty, arr);
        Console.WriteLine("Encryption exetime: {0}", s.ElapsedMilliseconds);
        return ret;
    ...
    public string Decrypt(string chipherText)
    {
        ...
        var val = Encoding.UTF8.GetString(Convert.FromBase64String(chipherText));

        var arr = new char[val.Length];
